...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || '(Eisbaeeer)';
    var email     = grunt.option('email')  || 'eisbaeeer@gmail.com';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new VR200 name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/VR200.png')) {
            fs.renameSync(__dirname + '/admin/VR200.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/VR200.html')) {
            fs.renameSync(__dirname + '/widgets/VR200.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/VR200/js/vr200.js')) {
            fs.renameSync(__dirname + '/widgets/VR200/js/vr200.js', __dirname + '/widgets/VR200/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/VR200')) {
            fs.renameSync(__dirname + '/widgets/VR200',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /VR200/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'VR200'
                        },
                        {
                            match: /Eisbaeeer/g,
                            replacement: author
